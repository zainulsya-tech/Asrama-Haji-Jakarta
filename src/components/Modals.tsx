...
export function Modals() {
  const { 
    ...
    checkoutRoom, cancelBooking, maintenances, showToast, qcInspections = [], workSessions = [], auditLogs = []
  } = useAppContext();
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100 animate-in fade-in zoom-in duration-150 flex flex-col max-h-[92vh] my-auto">
            ...
            {checkinData.actionType === 'CHECKIN' && !isAula && checkinMode === 'SELECT_BOOKING' && transactions.filter(t => t.roomId === room?.id && t.status === 'BOOKED').length > 0 ? (
              ...
              <form onSubmit={handleCheckin} className="p-6 space-y-4 text-xs overflow-y-auto flex-1 custom-scrollbar">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Tipe Tamu / Penyewa</label>
                  <div className="grid grid-cols-2 gap-2">
                    <label className="flex items-center space-x-2 p-2 rounded-lg border border-slate-200 cursor-pointer bg-slate-50 hover:bg-slate-100">
                      <input type="radio" name="guestCategory" value="JEMAAH" checked={category === 'JEMAAH'} onChange={() => setCategory('JEMAAH')} />
                      <span className="font-bold text-hajj-800"><i className="fa-solid fa-kaaba mr-1"></i> Jemaah Haji</span>
                    </label>
                    <label className="flex items-center space-x-2 p-2 rounded-lg border border-slate-200 cursor-pointer bg-slate-50 hover:bg-slate-100">
                      <input type="radio" name="guestCategory" value="UMUM" checked={category === 'UMUM'} onChange={() => setCategory('UMUM')} />
                      <span className="font-bold text-purple-800"><i className="fa-solid fa-user-group mr-1"></i> Tamu Umum / Penyewa</span>
                    </label>
                  </div>
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Nama Instansi / Nama Lengkap Tamu</label>
                  <input type="text" value={guestName} onChange={e => setGuestName(e.target.value)} required placeholder="Contoh: KBIHU Al-Hidayah / Bp. Hendra" className="w-full p-2.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-hajj-600" />
                </div>
                {category === 'JEMAAH' && (
                  <div>
                  ...
                  <form onSubmit={handleSaveUser} className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-4 text-xs">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      ...
                      <div>
                        ...
                        >
                          <optgroup label="Pimpinan & Super Admin">
                      ...
                      <div>
                        ...
                        {kamarTxs.length === 0 ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {kamarTxs.map(tx => (
                              <div key={tx.id} className="p-3.5 rounded-xl border border-emerald-200 bg-emerald-50/40 shadow-xs flex flex-col justify-between space-y-2">
                                <div>
                                  <div className="flex items-center justify-between mb-1.5">
                                    <div className="flex items-center space-x-1.5">
                                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-700 text-white">{tx.building}</span>
                                    ...
                                    <div className="flex flex-wrap items-center gap-1">
                                      ...
                                      {tx.breakfast ? (
                                        <span className="text-orange-700 font-bold bg-orange-100 px-1.5 py-0.5 rounded text-[10px]">
                                          {tx.breakfastMenu || 'Nasi Goreng'} ({tx.breakfastPortions || 1} Porsi × {tx.breakfastDays || tx.duration || 1} Hari)
                                          <span className="ml-1 text-[9px] px-1 py-0.2 rounded font-bold bg-white text-orange-800 border border-orange-200">
                                            {tx.breakfastStatus === 'SEDANG_DIBUAT' ? 'Dibuat' : tx.breakfastStatus === 'PENGANTARAN' ? 'Pengantaran' : tx.breakfastStatus === 'SELESAI' ? 'Selesai' : 'Menunggu'}
                                          </span>
                                        </span>
                                      ) : (
                                        <span className="text-slate-500 font-medium italic text-[10px]">Tidak Pesan</span>
                                      )}
                                    </div>
